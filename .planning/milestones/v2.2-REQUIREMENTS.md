...
- [x] **SEO-01**: `client/index.html` contains `<link rel="canonical" href="https://sunozip.com/" />` — verified present and regression-protected in build
- [x] **SEO-02**: Hero banner image converted to WebP at 121KB (<150KB threshold) — `client/src/assets/hero-banner.webp` replaces 2.4MB PNG
- [x] **SEO-03**: `public/sitemap.xml` includes the `/privacy` page URL; `lastmod` updated only when content actually changes (not on every deploy)
- [x] **SEO-04**: `client/index.html` includes `FAQPage` JSON-LD schema with 3–5 Q&A about the tool for AI Overview citation eligibility

### Batch Downloads (BAT)

- [x] **BAT-01**: `routes/download.js` uses `archiver` v7 streaming ZIP (piped directly to `res`) and `p-limit(8)` fetch concurrency — replaces `adm-zip` in-memory ZIP build; existing `/api/download/playlist` endpoint signature unchanged
- [x] **BAT-02**: When a playlist has more songs than `BATCH_SIZE`, the client automatically slices the selected songs into sequential batches and queues all downloads — each batch produces one ZIP named `PlaylistName-batch-01-of-N.zip`
- [x] **BAT-03**: SSE download progress events are emitted correctly — `sessionId` is included in the POST body so `global.downloadTrackers[sessionId]` can emit per-clip status updates
- [x] **BAT-04**: `BATCH_SIZE` defaults to 100 songs (raised from 50 in final commit) and is configurable via `VITE_BATCH_SIZE` env variable without code change
...
| BAT-01 | Phase 17 | Complete |
| BAT-02 | Phase 17 | Complete |
| BAT-03 | Phase 17 | Complete |
| BAT-04 | Phase 17 | Complete |
...
*Last updated: 2026-05-13 — v2.2 milestone complete (all 11 requirements fulfilled)*
